...
using the content-writing skill 先执行 AI 去痕（`humanizer` skill，无强度档位），再执行公众号文章预检、导流风险检查、违禁词合规检查。审阅未通过代表内容待调整，必须自动回步骤 3/4 改写并重新预检。产出：
- `$DIR/04-article-final.md` — 检查后的文章
- `$DIR/content-quality-report.md` — 含导流风险、内容完整性、标题摘要一致性、互动合规、违禁词、AI 痕迹检查；无待调整项后才能进入 SEO 与视觉
