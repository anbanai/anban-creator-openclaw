...
| **图像模型与参考策略** | 模型由任务 `image_model_key` 选定，`get_project_profile` 返回 `image_model{provider,model,key}`；按 provider 适配参考图：OpenAI/Gemini→全量产品图作 `ref_image_paths`（≤16）保真；Seedream→单锚点 + 产品档案文本块，多场景辅图改纯文生图规避雷同 |
...
- **`generate_image` 的参考图随任务 `image_model.provider` 而变**：OpenAI/Gemini→`ref_image_path`（单张）与 `ref_image_paths`（数组，≤16）可并用，电商优先把全部产品图作 `ref_image_paths` 保真；火山 Seedream→仅 `ref_image_path`（单张），强 i2i 不得复用同一参考于多张不同场景图
...
1. 产出 `$DIR/asset-plan.md`（仅含已选模块，逐张规划：用途/尺寸/视觉主体/必须出现卖点文字/禁用元素/参考图策略按 provider/验收标准）。
2. **锚点优先**：先生成主图①（点击主图）确立色系/版式/字体基准。
3. 按模块逐张生成：产品档案前缀块 + **按 `image_model.provider` 适配的参考图策略**（OpenAI/Gemini→把全部产品图作 `ref_image_paths` 多参考 ≤16 保真；Seedream→单 `$ANCHOR_REF` 用于主图①/详情核心场景等一致性关键图，场景差异大的辅图用纯文生图 + 共享风格块，**不重复使用同一参考图于多张不同场景图**规避雷同）+ `verify_with_vision` 自检，FAIL 强化约束重生成最多 3 轮，仍不达标标 `needs_reference`。
...
| **产品跨图不一致** | 产品档案锁定 + 锚点参考（OpenAI 多参考 / Seedream 单锚点）+ `verify_with_vision` 自检 + 3 轮收敛；仍不一致标 `needs_reference` |
| **多图复用同参考图导致场景雷同（Seedream 强 i2i）** | OpenAI 用多参考天然规避；Seedream 场景差异大的辅图改纯文生图 + 共享风格块，不重复使用同一参考图 |
