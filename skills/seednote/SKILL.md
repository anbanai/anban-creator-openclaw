...
using the seednote-visual-design skill，传入 `$DIR/content.md`，技能内部按 `seednote_image_mode` 完成图片内容规划（`$DIR/image-plan.md`）和全部图片生成。生成后检查每张图片：`$DIR/cover.png`（封面）、`$DIR/image_01.png` ...（仅含内容图的模式）、`$DIR/tail.png`（仅含尾图的模式）
