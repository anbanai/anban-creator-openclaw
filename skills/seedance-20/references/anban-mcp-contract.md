...
If `video-input-contract.json` says a video reference is required, that exact measured reference must remain in `generation_plan.references`; otherwise stop before provider dispatch.
